...
public:
  ...
  ll query(ll l, ll r, ll x) { // Assume 1 indexing for this problem, [) format
    ...
    for(l = l + eNum - 1, r = r + eNum - 1; l < r; l = l/2, r = r/2) {
      if(l % 2 == 1) {
        cout << "Looking inside st[" << l << "] : " << (upper_bound(st[l].begin(), st[l].end(), x) - st[l].begin()) << " found\n";
        res += (upper_bound(st[l].begin(), st[l].end(), x) - st[l].begin());
        l++;
      }
      if(r % 2 == 1) {
        r--;
        cout << "Looking inside st[" << r << "] : " << (upper_bound(st[r].begin(), st[r].end(), x) - st[r].begin()) << " found\n";
